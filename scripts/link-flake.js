...
// ...and near enough all of its failures have to have recovered on a later
// attempt. A host that fails every attempt is not flaky, it is broken, and
// ignoring it would hide a dead link rather than a dropped connection.
const CHRONIC_RECOVERY_RATE = 0.8;
...
function summarize(flags) {
  ...
  const lastFailures = new Set((perAttempt[attemptsRun - 1] || []).map((f) => f.url));
  ...
  for (const record of byUrl.values()) {
    // Independent of the run's overall outcome on purpose: a URL that failed
    // attempt 1 and passed attempt 3 flaked, even if some other URL was
    // genuinely dead and took the run down with it.
    record.recovered = !lastFailures.has(record.url);
  }
...
function aggregate(targets) {
  ...
  console.log(`Aggregated ${reports.length} report(s) across ${ranked.length} host(s).\n`);
  ...
  const chronic = ranked.filter(
    (entry) =>
      entry.runs >= CHRONIC_RUNS &&
      entry.failures > 0 &&
      entry.recovered / entry.failures >= CHRONIC_RECOVERY_RATE
  );

  console.log('');
  if (chronic.length === 0) {
    console.log(
      `No host flaked in ${CHRONIC_RUNS}+ runs while recovering on retry. ` +
        'Nothing to add to ignorePatterns.'
    );
    return;
  }
